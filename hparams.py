...
hparams = tf.contrib.training.HParams(
    ...
    pretrain_decoder_no_text=False,  # only train decoder with no text
    pretrain_decoder_little_text=True,  # only train decoder with little text, for removing mismatch
    decoder_text_rate=0.1,
    pretrain_decoder_text_random=False,
    decoder_random_rate=0.1,
    # only train decoder with little text and random context vector, for removing mismatch
    decoder_init_checkpoint='',  # pretrained model path, '.ckpt' file path
    # restore pretrained decoder & no restore whole model
    is_restore_pretrain_decoder=False,
    encoder_mode='no_text',  # donot mod this param, this is just for code:(
    ####
    # Audio
