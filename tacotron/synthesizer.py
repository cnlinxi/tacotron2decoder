...
class Synthesizer:
    def load(self, checkpoint_path, hparams, gta=False, model_name='Tacotron'):
        ...
        inputs_flag = tf.placeholder(tf.int32, (None), name='inputs_flag')
        ...
        with tf.variable_scope('Tacotron_model') as scope:
            ...
            if gta:
                self.model.initialize(inputs, input_lengths, targets, gta=gta, split_infos=split_infos,
                                      input_flag=inputs_flag)
            else:
                self.model.initialize(inputs, input_lengths, split_infos=split_infos, input_flag=inputs_flag)
    ...
    def synthesize(self, texts, basenames, out_dir, log_dir, mel_filenames):
        ...
        inputs_flag = [self.input_text for _ in seqs]
        ...
        feed_dict = {
            ...
            self.inputs_flag: np.asarray(inputs_flag, dtype=np.int32),
        }
